...
parser.add_argument('--num-symbols', type=int, default=7,
...
for step in range(args.iterations):
    ...
    # Load data
    fname = f'/nlp/scr/rewang/rl-starter-files/trajectories/{step}_data.pkl'
    with open(fname, 'rb') as f:
        data = pickle.load(f)

    # Add 1 to actions because 0 is EOS
    data = [torch.cat((d + 1, torch.tensor([0]))) for d in data]

    lengths = torch.tensor(list(map(len, data))) # account for EOS
